...
import {
  ...
  ArrowLeft, LayoutDashboard, Boxes, ClipboardList,
  Ticket, Users, ChevronRight
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-orange-500 selection:text-white">
      ...
      <div className="flex min-h-screen">
        ...
        {/* ── SIDEBAR ────────────────────────────────────────── */}
        <aside className="w-64 flex-shrink-0 bg-slate-900 border-r border-slate-800 flex flex-col py-6 px-4 gap-2 sticky top-0 h-screen overflow-y-auto">
          <Link to="/" className="flex items-center gap-2.5 mb-6 px-2">
            ...
            <div>
              <span className="text-base font-extrabold text-white">Food<span className="text-orange-500">Delivery</span></span>
              <span className="block text-[10px] text-orange-400 font-bold uppercase tracking-wider">Admin Panel</span>
            </div>
          </Link>

          {[
            { to: '/admin/dashboard', icon: <LayoutDashboard className="w-4 h-4" />, label: 'Dashboard', active: true },
            { to: '/admin/orders',    icon: <ClipboardList  className="w-4 h-4" />, label: 'Đơn hàng' },
            { to: '/admin/products',  icon: <Boxes          className="w-4 h-4" />, label: 'Sản phẩm' },
            { to: '/admin/promotions',icon: <Ticket         className="w-4 h-4" />, label: 'Khuyến mãi' },
            { to: '/admin/users',     icon: <Users          className="w-4 h-4" />, label: 'Khách hàng' },
          ].map(item => (
            <Link
              key={item.to}
              to={item.to}
            ...
            >
              <span className={item.active ? 'text-orange-400' : 'text-slate-500 group-hover:text-slate-300'}>
                {item.icon}
              </span>
              {item.label}
              {item.active && <ChevronRight className="w-3 h-3 ml-auto text-orange-400" />}
            </Link>
          ))}
